...
## Procedure
1. Tooling: prefer `gitleaks detect --source . --report-format json --report-path /tmp/gitleaks.json` if `command -v gitleaks` succeeds; else `trufflehog git file://. --json`; else fall back to grep over `git log -p --all` with the pattern set below.
...
5. Remediation plan, in this order, as a checklist for the user:
   ...
   4. Add prevention: `.gitignore` entries, pre-commit hook (`gitleaks protect --staged`), and this plugin's `secret-scan` hook.
